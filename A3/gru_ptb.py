...
import mod_rnn_cell as rnn_cell
import reader
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    lstm_cell = rnn_cell.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True,omit=FLAGS.omit)
    gru_cell = rnn_cell.GRUCell(size)
    #gru_cell=lstm_cell
    if is_training and config.keep_prob < 1:
      gru_cell = rnn_cell.DropoutWrapper(
          gru_cell, output_keep_prob=config.keep_prob)
    cell = rnn_cell.MultiRNNCell([gru_cell] * config.num_layers,state_is_tuple=False)
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as session:
      ...
      if FLAGS.save_path:
        with open(os.path.join(FLAGS.save_path,'test_perp.txt'),'w') as f:
