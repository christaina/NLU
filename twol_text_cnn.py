...
class TextCNN(object):
    ...
    def __init__(
      ...
      l2_reg_lambda=0.0):
        ...
        pooled_outputs_t = self.convpool(self.embedded_chars_expanded, filter_sizes, \
                embedding_size, num_filters,30)
        self.h_pool_layer1 = tf.concat(3,pooled_outputs_t)
        hshape = self.h_pool_layer1.get_shape()
        self.h_pool_layer1 = tf.reshape(self.h_pool_layer1,[-1,int(hshape[1]),\
                int(hshape[3]),1])
        # Combine all the pooled features
        num_filters_total = num_filters * len(filter_sizes)
        pooled_outputs_2 = self.convpool(self.h_pool_layer1,filter_sizes_2, num_filters_total, num_filters_2, self.h_pool_layer1.get_shape()[1])
